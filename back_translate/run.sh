...
# a separate line
input_file=COVID19_FAQ.csv

# sampling_temp: The sampling temperature for translation.
# Lower (e.g., 0.5) = less diverse paraphrases, Higher (e.g., 1) = more diverse
sampling_temp=0.4

# iterations: the number of paraphrasing copies of the input_file to make
iterations=3
...
do
  ...
  # install nltk
  python split_paragraphs_csv.py \
  ...
  echo "*** move paraphrases to .csv copy ***"
  python para_to_csv.py \
    --input_file=${para_dir}/file_${worker_id}_of_${replicas}.txt \
    --orig_csv=${input_file} \
    --output_file=${para_dir}/paraphrased_${input_file}
done 

echo "*** appending original document to paraphrased document ***"
python final_csv.py \
  --input_file=${para_dir}/paraphrased_${input_file} \
  --orig_csv=${input_file}