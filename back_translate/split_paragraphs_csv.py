...
def main(_):
  ...
  inf = pd.read_csv(FLAGS.input_file)
  contents = list(inf.iloc[:,1])  # select second column of input_file .csv
  ...
  with open(FLAGS.output_file, "w") as ouf:
    for st in new_contents:
      ouf.write(st + "\n")
  with open(FLAGS.doc_len_file, "w") as ouf:
