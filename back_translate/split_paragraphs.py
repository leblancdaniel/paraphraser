...
def main(_):
  ...
  with tf.compat.v1.gfile.Open(FLAGS.input_file) as inf:  # read input file
    contents = inf.readlines()  # convert .txt lines to list
  ...
  with tf.compat.v1.gfile.Open(FLAGS.output_file, "w") as ouf:
    for st in new_contents:
      ouf.write(st + "\n")
  with tf.compat.v1.gfile.Open(FLAGS.doc_len_file, "w") as ouf:
